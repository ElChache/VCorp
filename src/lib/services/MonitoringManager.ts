		...
		return messages
			...
			.map(msg => {
				...
				if (msg.isReply && msg.parentContentId) {
					...
					return `• [${authorPrefix}] THREAD UPDATE (reply):
  📝 Original: ${msg.title || '(no title)'}
  ↳ Reply: ${msg.body}
  
  🔧 Simple function to check thread context:
  ${threadCommand}
  
  🔧 Simple function to reply:
  ${replyCommand} "Your response here"`;
				} else {
					// Regular message - show complete content
					return `• [${authorPrefix}] ${msg.title || msg.type}:
  ${msg.body}
